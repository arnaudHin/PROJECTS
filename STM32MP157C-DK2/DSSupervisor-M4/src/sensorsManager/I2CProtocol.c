...
#include <sensorsManager/I2CProtocol.h>
...
extern DavidsonStatus_e I2CProtocol_GetSensorMeasure(SensorId_e sensorId){
	...
	switch (sensorId) {
		...
		case HUMI:
			...
			Humidity_t * humidityTemp = (Humidity_t*) Humidity_Get();
			uint8_t adressSlaveHumidRead = humidityTemp->humidityAdress;

			uint8_t register_humid_H = humidityTemp->humidityRegValueHigh;
			uint8_t register_humid_L = humidityTemp->humidityRegValueLow;

			uint8_t dataHumid_H = 0;
			uint8_t dataHumid_L = 0;

			uint8_t dataHumid[2];

			status = I2CProtocol_Read(adressSlaveHumidRead, register_humid_H, &dataHumid_H);
			status = I2CProtocol_Read(adressSlaveHumidRead, register_humid_L, &dataHumid_L);

			dataHumid[0] = dataHumid_H;
			dataHumid[1] = dataHumid_L;
