...
void SPI_Init(SPI_TypeDef* SPIx)
{
	...
	if(id == SPI1_ID)
	{
		...
		BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_5, GPIO_MODE_AF_PP, GPIO_PULLUP, GPIO_SPEED_HIGH,GPIO_AF5_SPI1);
		BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_6, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_HIGH,GPIO_AF5_SPI1);
		BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_7, GPIO_MODE_AF_PP, GPIO_PULLUP, GPIO_SPEED_HIGH,GPIO_AF5_SPI1);
	...
	else
	{
		...
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_13, GPIO_MODE_AF_PP, GPIO_PULLUP, GPIO_SPEED_HIGH,GPIO_AF5_SPI2);
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_14, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_HIGH,GPIO_AF5_SPI2);
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_15, GPIO_MODE_AF_PP, GPIO_PULLUP, GPIO_SPEED_HIGH,GPIO_AF5_SPI2);
	}
...
void SPI_WriteAndReadNoRegister(SPI_TypeDef* SPIx, uint8_t * Rxdata, uint8_t Txdata)
{
	assert(SPIx == SPI1 || SPIx == SPI2);
	SPI_ID_e id = ((SPIx == SPI2)?SPI2_ID:SPI1_ID);
...
void mon_code(void)
{
	...
	tx[0] = 0x32;
	tx[1] = 28;
	SPI_WriteAndReadMultiNoRegister(SPI2, rx, tx, 2);

	uint8_t rxbyte;
	uint8_t txbyte = 0xCA;
	SPI_WriteAndReadMultiNoRegister(SPI2, &rxbyte, &txbyte, 1);
}

void SPI_WriteAndReadMultiNoRegister(SPI_TypeDef* SPIx, uint8_t * Rxdata, uint8_t * Txdata, uint16_t count)
{
	assert(SPIx == SPI1 || SPIx == SPI2);
	SPI_ID_e id = ((SPIx == SPI2)?SPI2_ID:SPI1_ID);
